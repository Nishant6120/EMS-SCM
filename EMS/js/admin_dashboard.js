(function($) {
    ...
    // Formats a Date object or 'dd/mm/yyyy' string to 'yyyy-mm-dd'.
        ...
        try {
            ...
            $(campaignsTableBody).off('click').on('click', '.toggle-master-status-btn', async (e) => {
                ...
                if (campaignToEdit) {
                    ...
                    // Set up cascading for process select in the modal
    ...
    function applyFiltersAndRenderTasks() {
        ...
        if (currentSectionId === 'all-tasks-section') {
            searchTerm = $('#universalTaskSearchInput').val();
        } else if (currentSectionId === 'view-update-tasks-section') {
            searchTerm = $('#viewTaskSearchInput').val();
        }
    ...
    function populateEditTaskModal(task) {
        ...
        $('#editOrganization').on('change.editTaskCascade', function() {
            ...
            const $editLeadsCampaignSelect = $('#editLeadsCampaign');
            const $editProspectsCampaignSelect = $('#editProspectsCampaign');
            ...
            if (orgId) {
                ...
                if (filteredEmployees.length > 0) {
                    ...
                    $editEmployeeSelect.empty().append('<option value="">No Employees Found</soption>').val('').trigger('change.select2');
                }
            }
        });
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    $('#editLeadsCampaign').val((task.leads && task.leads.campaign) || '').trigger('change.select2');
                    $('#editProspectsCampaign').val((task.prospects && task.prospects.campaign) || '').trigger('change.select2');
                }, 50);
            }, 50);
        }, 50);
    ...
    function renderActiveFilters() {
        ...
        let searchInput;
        if (currentSectionId === 'all-tasks-section') {
            searchInput = $('#universalTaskSearchInput');
        } else if (currentSectionId === 'view-update-tasks-section') {
            searchInput = $('#viewTaskSearchInput');
        ...
        $(activeFiltersDisplay).off('click', '.remove-filter').on('click', '.remove-filter', function() {
            ...
            if (keyToRemove === 'searchTerm') {
                ...
                $('#universalFilterForm').find(`#filter${keyToRemove.charAt(0).toUpperCase() + keyToRemove.slice(1)}`).val(null).trigger('change.select2');
            }
            applyFiltersAndRenderTasks();
        });
    }
    ...
    function setupCreateTaskCascades() {
        ...
        $('#taskOrganizationSelect').off('change.taskFilter').on('change.taskFilter', function() {
            ...
            const $leadsCampaignSelect = $('#leadsQuantityCampaignMaster');
            const $prospectsCampaignSelect = $('#prospectsQuantityCampaignMaster');
    ...
    function renderAllocatedTasksAdmin() {
        ...
        $(elements.allocatedTasksTableBody).off('click').on('click', '.toggle-allocated-task-status-btn', function() {
            const taskId = $(this).data('id');
            const newStatus = $(this).data('status');
    ...
    function populateAllocatedTaskEditModal(task) {
        ...
        const employeeName = employees.find(e => e.id === task.assignedTo)?.name || '--';
        $('#editAllocatedTaskEmployeeSelect').empty().append(new Option(employeeName, task.assignedTo)).trigger('change.select2');
    ...
    function renderHourlyUpdatesAdminTable() {
        const $tableBody = $('#hourlyUpdatesAdminTableBody');
        $tableBody.empty();
        ...
        sortedUpdates.forEach(entry => {
            const employeeName = employees.find(emp => emp.id === entry.employeeId)?.name || '--';
            const processName = processes.find(proc => proc._id === entry.processId)?.name || '--';
    ...
    function showSection(sectionId, updateUrl = true) {
        ...
        const $targetNavLink = $(`#main-admin-navbar .nav-link[data-section="${sectionId}"]`);
        const $targetDropdownItem = $(`#main-admin-navbar .dropdown-item[data-section="${sectionId}"]`);

        if ($targetNavLink.length) {
            $targetNavLink.addClass('active');
        } else if ($targetDropdownItem.length) {
            $targetDropdownItem.addClass('active');
            $targetDropdownItem.closest('.dropdown').find('.dropdown-toggle').addClass('active');
        } else {
            $('#main-admin-navbar .nav-link[data-section="manage-user"]').addClass('active');
        }
    ...
    $(document).ready(() => {
        ...
        $('#main-admin-navbar .nav-link, #main-admin-navbar .dropdown-item').on('click', function(e) {
            ...
            if (targetSection) {
                ...
                console.warn("Clicked nav element has no data-section attribute:", this);
            }
        });
        ...
        $('#saveProcessChangesBtn').on('click', async function() {
            ...
            if (!newName || !newOrganizationId) {
                alert('Process Name and Organization cannot be empty.');
                return;
            }
        ...
        $('#saveCampaignChangesBtn').on('click', async function() {
            ...
            if (!newName || !newOrganizationId || !newProcessId) {
                alert('Campaign Name, Organization, and Process are required.');
                return;
            }
        ...
        $('#createTaskForm').on('submit', async function(e) {
            ...
            const taskFinalData = {
                ...
                taskDate: formatDateToISO(taskDate),
                status: 'Pending',
        ...
        $('#saveTaskChangesBtn').on('click', async function() {
            ...
            const taskUpdatePayload = {
                ...
                taskDate: formatDateToISO(updatedTaskDate),
                generalComment: '', 
                leads: leadsDataPayload,
                prospects: prospectsDataPayload,
        ...
        $('#all-tasks-section #universalTaskSearchInput').on('input', function() {
            currentFilters.searchTerm = $(this).val();
            applyFiltersAndRenderTasks();
        });
        
        // Event handler for the "All Tasks" clear search button
        $('#all-tasks-section #clearUniversalSearchBtn').on('click', function() {
            $('#all-tasks-section #universalTaskSearchInput').val('');
            currentFilters.searchTerm = '';
            applyFiltersAndRenderTasks();
        });

        // Event handler for the "View Task Status" search input
        $('#view-update-tasks-section #viewTaskSearchInput').on('input', function() {
            currentFilters.searchTerm = $(this).val();
            applyFiltersAndRenderTasks();
        });

        // Event handler for the "View Task Status" clear search button
        $('#view-update-tasks-section #clearViewSearchBtn').on('click', function() {
            $('#view-update-tasks-section #viewTaskSearchInput').val('');
            currentFilters.searchTerm = '';
            applyFiltersAndRenderTasks();
        });

        // Event handler for the Filter Offcanvas Submit button
        ...
        // Event handler for the 'Clear All' buttons in both sections
        ...
        // Event handlers for moving items between dual list boxes in the 'Create Task' section
        $('#create-task-section').on('click', '#leadsAddDateRangeBtn', () => addAssignment('dateRange', 'leads'));
        $('#create-task-section').on('click', '#leadsAddMonthYearBtn', () => addAssignment('monthYear', 'leads'));
        $('#create-task-section').on('click', '#leadsAddRegionBtn', () => addAssignment('region', 'leads'));
        $('#create-task-section').on('click', '#leadsAddCampaignBtn', () => addAssignment('campaign', 'leadsQuantity'));
        $('#create-task-section').on('click', '#prospectsAddDateRangeBtn', () => addAssignment('dateRange', 'prospects'));
        $('#create-task-section').on('click', '#prospectsAddMonthYearBtn', () => addAssignment('monthYear', 'prospects'));
        $('#create-task-section').on('click', '#prospectsAddRegionBtn', () => addAssignment('region', 'prospects'));
        $('#create-task-section').on('click', '#prospectsAddCampaignBtn', () => addAssignment('campaign', 'prospectsQuantity'));
        ...
        $('#prospectsMoveLeftBtn', () => moveOptions('rightBoxProspects', 'leftBoxProspects'));

        // Event handlers for moving items in the 'Edit Task' modal
        $('#taskEditModal').on('click', '#editLeadsAddDateRangeBtn', () => addAssignment('dateRange', 'editLeads'));
        $('#taskEditModal').on('click', '#editLeadsAddMonthYearBtn', () => addAssignment('monthYear', 'editLeads'));
        $('#taskEditModal').on('click', '#editLeadsAddRegionBtn', () => addAssignment('region', 'editLeads'));
        $('#taskEditModal').on('click', '#editLeadsAddCampaignBtn', () => addAssignment('campaign', 'editLeads'));
        $('#taskEditModal').on('click', '#editProspectsAddDateRangeBtn', () => addAssignment('dateRange', 'editProspects'));
        $('#taskEditModal').on('click', '#editProspectsAddMonthYearBtn', () => addAssignment('monthYear', 'editProspects'));
        $('#taskEditModal').on('click', '#editProspectsAddRegionBtn', () => addAssignment('region', 'editProspects'));
        $('#taskEditModal').on('click', '#editProspectsAddCampaignBtn', () => addAssignment('campaign', 'editProspects'));
        ...
        $('#editProspectsMoveLeftBtn', () => moveOptions('rightBoxEditProspects', 'leftBoxEditProspects'));
